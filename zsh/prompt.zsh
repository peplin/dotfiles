...
git_branch() {
  echo $(timeout .2 $git symbolic-ref HEAD 2>/dev/null | awk -F/ {'print $NF'})
}

git_prompt_info() {
  ref=$(timeout .2 $git symbolic-ref HEAD 2> /dev/null) || return
  echo "${ref#refs/heads/}"
}

git_dirty() {
    st=$(timeout .2 $git status 2>/dev/null | tail -n 1)
    if [[ $st == "" ]]
    then
        echo ""
    else
...
unpushed () {
  timeout .2 $git cherry -v @{upstream} 2>/dev/null
}
